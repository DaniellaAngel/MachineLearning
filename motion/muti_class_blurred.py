...
y_pridect = tf.nn.softmax(tf.matmul(h_fc1_drop, W_fc2) + b_fc2)


cross_entropy = -tf.reduce_sum(Y*tf.log(y_pridect))
###########learning rate decay
...
correct_prediction = tf.equal(tf.argmax(y_pridect,1), tf.argmax(Y,1))
accuracy = tf.reduce_mean(tf.cast(correct_prediction, 'float'))

train_batch, train_label_batch = tf.train.batch([train_imgs, _train_labels],batch_size=BATCH_SIZE, capacity=32)
...
test_batch,test_label_batch = tf.train.batch([test_imgs, _test_labels],batch_size=BATCH_SIZE, capacity=32)
